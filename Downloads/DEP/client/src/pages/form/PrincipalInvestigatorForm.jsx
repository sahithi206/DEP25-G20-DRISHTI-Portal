      ...
      reader.onload = (e) => {
        try {
          ...
          setInvestigators((prevInvestigators) => {
            let updatedInvestigators = [...prevInvestigators];
            let jsonIndex = 0;
  
            // Fill existing empty forms first
            updatedInvestigators = updatedInvestigators.map((member) => {
              if (jsonIndex < jsonData.members.length && !member.name) {
                return { ...jsonData.members[jsonIndex++] };
              }
              return member;
            });
  
            // Add new members only if JSON has more data than empty slots
            while (jsonIndex < jsonData.members.length) {
              updatedInvestigators.push(jsonData.members[jsonIndex++]);
            }
