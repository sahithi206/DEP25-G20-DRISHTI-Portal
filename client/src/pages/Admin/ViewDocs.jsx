...
const ViewDocs = () => {
    ...
    const fetchPendingSEs = async () => {
        try {
            const response = await fetch(`${url}admin/seforms/${id}?t=${Date.now()}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    "accessToken": localStorage.getItem("token"),
            ...
            <div className="flex-1 p-6 overflow-y-auto">
                ...
                <div className="p-6 space-y-1 mt-3">
                    <div key={reloadKey} className="mt-3 bg-white p-7 rounded-lg shadow-md">
                        <div>
                            <h2 className="text-lg font-bold mb-4">Pending UCs</h2>
                            ...
                            <h2 className="text-lg font-bold mt-8 mb-4">Pending SEs</h2>
                            <table className="w-full border table-fixed">
                                <thead>
                                    <tr className="bg-gray-200">
                                        <th className="p-4 text-left w-1/4">Certificate ID</th>
