...
const AuthProvider = (props) => {
  ...
  const approvedProjects = async () => {
    ...
    try {
      ...
      const json = await response.json();
      if (!json.success) {
        alert(json.msg || "No projects found.");
        return [];
      }
      return json.data || [];
    } catch (error) {
  ...
  return (
    <AuthContext.Provider value={{ logout, sendOtp, verifyOtp, login, authState, submitProposal, submitGeneralInfo, submitResearchDetails, submitBudgetDetails, submitBankDetails, submitPIDetails, submitAcknowledgement, getuser, approvedProjects }}>
      {props.children}
