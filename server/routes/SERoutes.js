...
router.get("/:id", async (req, res) => {
    try {
        const se = await SERequest.findById(req.params.id);
        
        if (!se) {
            return res.status(404).json({ success: false, message: "Statement of Expenditure not found" });
        }
        
        res.json({ success: true, data: se });
    } catch (err) {
        console.error("Error fetching SE:", err);
